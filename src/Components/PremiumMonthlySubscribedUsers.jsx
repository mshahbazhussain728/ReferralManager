...
// // //   // ✅ On mount: read selected user from router state → localStorage → fallback to first
// // //   useEffect(() => {
...
// // //   // ✅ Current user = the one selected in dashboard
// // //   const currentUser = allLeftUsers.find((u) => u.name === selectedUserName) || allLeftUsers[0];

// // //   // ✅ Left panel: only show the selected user (search filters within left panel but only that user shows)
// // //   const filteredLeftUsers = allLeftUsers
...
// // //   const thisMonthUsers = filteredTableData.filter(user =>
// // //     user.date.includes("Feb") && user.date.includes("2025")
// // //   ).length;
...
// // //   ];
// // //   const thisWeekUsers = filteredTableData.filter(user =>
// // //     thisWeekDates.includes(user.date)
// // //   ).length;
...
// // //         {/* ✅ Only the selected user is shown */}
...
// //   // ── Read ALL user info from navigation state (passed by MarketingAgents) ──
...
// //   // ── API 1: Stats → monthlySubscribedUsers (only used for thisMonth, thisWeek, revenue) ──
...
// //       const res = await fetch(`${BASE_URL}/referrals/users/details/get`, {
...
// //       if (json.success && json.data?.monthlySubscribedUsers) {
// //         const m = json.data.monthlySubscribedUsers;
// //         setStats({
...
// //   // ── API 2: Table → monthly + active ──────────────────────────────────────
// //   const [tableData, setTableData]       = useState([]);
// //   const [totalRecords, setTotalRecords] = useState(0);  // ✅ source of truth for total users
...
// //         setTotalRecords(json.pagination?.total    || 0);  // ✅ actual count from API 2
...
// //     parseInt(localStorage.getItem('permanent_count_paymentRequests'), 10) || null
// //   }
// //   marketingAgentsCount={
// //     parseInt(localStorage.getItem('permanent_count_marketingAgents'), 10) || null
// //   }
// // />
...
// //               </div>
// //               <div className="flex items-center justify-start gap-2 text-xs ml-[48px] mt-[-22px]">
// //                 {/* ✅ Users count from API 2 totalRecords, not stats.total */}
...
// //                 })
// //               ) : (
// //                 <tr><td colSpan="4" className="p-4 text-center text-gray-500">No subscribed users found</td></tr>
// //               )}
// //             </tbody>
// //           </table>
// //         </div>
...
// //   // ── Read ALL user info from navigation state ──────────────────────────────
// //   const selectedUserId =
// //     location.state?.selectedUserId ||
// //     parseInt(localStorage.getItem("selectedUserId")) ||
// //     null;
...
// //                 </span>
// //                 <span className="text-gray-500">
// //                   Balance: <span className="text-[#055860]">${parseFloat(selectedUserBalance).toLocaleString()}</span>
// //                 </span>
// //               </div>
// //             </div>
// //           ) : (
...
//       sessionStorage.setItem("freeUsersState", JSON.stringify(location.state));
//     }
//   }, [location.state]);
...
//     : (() => {
//         try {
//           const s = sessionStorage.getItem("freeUsersState");
//           return s ? JSON.parse(s) : {};
//         } catch { return {}; }
//       })();

//   const selectedUserId       = savedState.selectedUserId      || null;
//   const selectedUserName_nav = savedState.selectedUserName    || "User";
...
//   const showLeftUser = selectedUserName_nav.toLowerCase().includes(userSearchTerm.toLowerCase());
...
//                   <img
//                     src={selectedUserAvatar}
//                     alt={selectedUserName_nav}
//                     onError={(e) => {
//                       e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(selectedUserName_nav)}&background=055860&color=fff`;
//                     }}
//                     className="w-9 h-9 rounded-full object-cover mt-[13px] mr-[-5px]"
//                   />
...
//                 </span>
//                 <span className="text-gray-500">
//                   Balance: <span className="text-[#055860]">${parseFloat(selectedUserBalance || 0).toLocaleString()}</span>
//                 </span>
//               </div>
//             </div>
//           ) : (
