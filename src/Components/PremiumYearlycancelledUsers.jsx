...
export default function YearlyCancelledUsers() {
  ...
  const handlePrevPage = () => {
    if (currentPage > 1) { const p = currentPage - 1; setCurrentPage(p); fetchTableData(p); }
  };
  const handleNextPage = () => {
    if (currentPage < totalPages) { const p = currentPage + 1; setCurrentPage(p); fetchTableData(p); }
  };
  const handlePageClick = (p) => {
    if (p !== currentPage) { setCurrentPage(p); fetchTableData(p); }
  };
        ...
        <div className="flex-1 space-y-2 overflow-y-auto mt-[-15px]">
          {showLeftUser ? (
            <div className="flex flex-col p-2 mt-3 rounded-md bg-[#E8F0F6] border border-[#055860]">
              ...
              <div className="flex items-center justify-start gap-2 text-xs ml-[48px] mt-[-22px]">
                <span className="text-gray-500">
                  Users: <span className="text-[#055860]">{totalRecords}</span>
                </span>
                {/* ✅ Now shows selectedUserBalance from navigation state */}
                <span className="text-gray-500">
                  Balance:{" "}
                  <span className="text-[#055860]">
          ...
          <div className="flex-1 border border-gray-300 rounded-lg overflow-hidden">
            <div className="flex">
              ...
              <div className="flex flex-1">
                ...
                >
                  <div className="text-lg font-semibold leading-[70%]">
                    {statsLoading ? "..." : `$${Number(stats.totalrevenue).toLocaleString()}`}
                  </div>
                  <div className="text-md opacity-90 mt-[14px]">Revenue</div>
                </div>
              </div>
