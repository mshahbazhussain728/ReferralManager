...
// //   // ===== CRITICAL: SIDEBAR COUNT CONFIGURATION =====
// //   // ✅ ALWAYS 12 - static from free-all-users page (NEVER CHANGES)
// //   const totalFreeUsers = 12;
// //   // ✅ ALWAYS 0 on this page (not premium page)
...
//                 {/* Bottom row: total table users count + balance */}
//                 <div className="flex items-center justify-start gap-2 text-xs ml-[48px] mt-[-22px]">
//                   <span className="text-gray-500">
...
export default function FreeUsers() {
  ...
  // ✅ FIX: Left panel only shows the ONE selected user (search filters within that single user)
  const selectedUser = allLeftUsers.find((u) => u.name === selectedUserName) || allLeftUsers[0];

  // The left panel list: only the selected user, optionally filtered by userSearchTerm
  const leftPanelUsers = allLeftUsers
    .filter(u => u.name.toLowerCase().includes(userSearchTerm.toLowerCase()))
    .sort((a, b) => b.earning - a.earning)
    .filter(u => u.name === selectedUser.name); // ✅ Only show the selected user
  ...
  // Stats
  const totalUsers = filteredTableData.length;
  const thisMonthUsers = filteredTableData.filter(u => u.date.includes("Jan") && u.date.includes("2025")).length;
  ...
  return (
    <div className="h-[1000px] min-h-screen flex bg-[#F5F6FA]">
      ...
      <div className="h-[980px] w-[290px] bg-white rounded-md shadow-sm p-2 flex flex-col relative ml-[16px] mt-[10px]">
        ...
        {/* ✅ Left panel only shows the selected user */}
        <div className="flex-1 space-y-2 overflow-y-auto mt-[-15px]">
