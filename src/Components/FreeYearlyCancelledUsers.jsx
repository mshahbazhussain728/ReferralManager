...
// //           {/* First box - Back Arrow - Now uses navigate(-1) to go back */}
// //           <button 
// //             onClick={() => navigate(-1)} 
...
//   // const RenderStars = (rating) => {
//   //   return Array.from({ length: 5 }, (_, i) => (
//   //     <span key={i} style={{ color: i < rating ? "#691188" : "#d1d5db" }}>★</span>
//   //   ));
//   // };
...
//   const thisWeekUsers = filteredTableData.filter(user =>
//     ["Jan 14 2025","Jan 15 2025","Jan 16 2025","Jan 17 2025","Jan 18 2025","Jan 19 2025","Jan 20 2025"]
//       .includes(user.date)
//   ).length;
...
//                   <span className="text-xs text-[#055860] mt-3">{user.country}</span>
//                 </div>
//                 {/* Users = actual tableData.length | Balance = user.balance */}
//                 <div className="flex items-center justify-start gap-2 text-xs ml-[48px] mt-[-22px]">
...
export default function YearlyCancelledUsers() {
  ...
  // ✅ On mount: read from router state → localStorage → fallback to first user
  useEffect(() => {
  ...
  const selectedUser = leftUsers.find((u) => u.name === selectedUserName) || leftUsers[0];
  const currentUser = selectedUser;

  // ✅ Only show the selected user in the left panel (filtered by search)
  const leftPanelUsers = leftUsers
  ...
  const totalFreeUsers = leftUsers.length;
  const totalPremiumUsers = 13;
  const totalMarketingAgents = 10;
  const totalPaymentRequests = 16;
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex gap-[2px] ml-1 mt-[-10px]"></div>
                  </div>
                  <span className="text-xs text-[#055860] mt-3">{user.country}</span>
                </div>
                <div className="flex items-center justify-start gap-2 text-xs ml-[48px] mt-[-22px]">
